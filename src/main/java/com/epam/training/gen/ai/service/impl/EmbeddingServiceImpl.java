...
public class EmbeddingServiceImpl implements EmbeddingService {
    ...
    public Flux<List<Float>> buildEmbedding(String input) {
        ...
        return openAiClient.getEmbeddings(properties.getDeploymentName(), options).flatMapIterable(Embeddings::getData)
    ...
    public Flux<String> storeEmbedding(String input) {
        return buildEmbedding(input)
                .map(vector -> PointStruct.newBuilder().setId(id(UUID.nameUUIDFromBytes(input.getBytes()))).setVectors(vectors(vector)).putPayload("text", value(input))
                        .build()).buffer().map(points -> qdrantClient.upsertAsync(properties.getCollectionName(), points))
    ...
    private SearchPoints createSearchPoints(List<Float> vector) {
        return SearchPoints.newBuilder().setCollectionName(properties.getCollectionName()).addAllVector(vector).setLimit(properties.getSearchLimit())
