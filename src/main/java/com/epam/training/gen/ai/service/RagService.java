...
public class RagService {
    ...
    public Mono<String> getResponse(String prompt) throws ServiceNotFoundException {
        ...
        return chatCompletionService.getChatMessageContentsAsync(chatHistory, semanticKernel, invocationContext).doOnNext(this::updateHistory).map(RagService::convertMessagesToString).doOnNext(response -> log.info("""
                Input prompt: ```
                ${prompt}
                ```
                Model response: ```
