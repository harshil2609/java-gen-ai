...
public class QdrantInitializer {
    ...
    private void ensureCollectionExists() throws InterruptedException, ExecutionException {
        ...
        if (!exists) {
            client.createCollectionAsync(properties.getCollectionName(), VectorParams.newBuilder().setDistance(Distance.Cosine).setSize(properties.getVectorSize()).build()).get();
